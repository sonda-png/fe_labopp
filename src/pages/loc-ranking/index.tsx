...
import type { LocRankingResponse, StudentRankingData } from "@/api/actions/loc-ranking/loc-ranking.type"

export default function LOCRankingSystem() {
  const search = useSearch({ from: LOCRankingRoute.id })
  const [selectedStudent, setSelectedStudent] = useState<StudentRankingData | null>(null)
  const [searchTerm, setSearchTerm] = useState("")

  const { data: rankingData, isLoading, error } = useQuery<LocRankingResponse>(
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center space-y-4">
          ...
          <p className="text-muted-foreground">Could not load ranking data. Please try again later.</p>
        </div>
      </div>
    )
  }
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">View and track student LOC rankings for class {search.classId}</p>
            </div>
          </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        ...
                        <div className="font-semibold text-gray-900">{student.totalLOC}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-sm text-gray-500">Passed Assignments</div>
                        <div className="font-semibold text-gray-900">{student.passedAssignments}</div>
                      </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-gray-50 p-4 rounded-lg">
                  <div className="text-sm text-gray-500">Total LOC</div>
                  <div className="text-2xl font-bold text-gray-900">{selectedStudent.totalLOC}</div>
                </div>
                <div className="bg-gray-50 p-4 rounded-lg">
                  <div className="text-sm text-gray-500">Passed Assignments</div>
                  <div className="text-2xl font-bold text-gray-900">{selectedStudent.passedAssignments}</div>
                </div>
              </div>
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="text-sm text-gray-500 mb-2">Current Rank</div>
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${getRankBadgeColor(selectedStudent.rank)}`}>
                    {getRankIcon(selectedStudent.rank)}
                  </div>
                  <div className="text-2xl font-bold text-gray-900">Rank #{selectedStudent.rank}</div>
                </div>
              </div>
            </div>
          )}
