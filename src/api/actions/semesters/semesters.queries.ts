...
export const semestersQueries = {
  ...
  getAll: () =>
    queryFactoryOptions({
      queryKey: [...semestersQueries.all(), 'list'],
      queryFn: getAllSemesters,
      enabled: true,
    }),
  getSemesterByClass: (params: SemesterByClassRequest) =>
    queryFactoryOptions({
      queryKey: [...semestersQueries.all(), 'class', params],
...
const getSemesterByClass =
  (params?: SemesterByClassRequest) => (client: AxiosInstance) => async () => {
    return (
      await client.get<Semester>(
        `/head_subject/semester/classes${objectToQueryString(params ?? {})}`
      )
    ).data
  }
